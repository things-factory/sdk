...
describe('typescript project references', () => {
  ...
  const references = referencesConfig.references.map(({ path }) => path.replace('./', ''))
  const thingsFactoryReferences = references.map(prefixPackageName)
  ...
  it('includes all the packages', () => {
    const packages = glob
      ...
      .map(
        packageJsonPath =>
          /sdk\/packages\/(?<packageName>[\w._-]+)\/package\.json$/i.exec(packageJsonPath).groups.packageName
      )
  ...
  references.map(packageName => {
    ...
    describe(`${displayedName}`, () => {
      it(`includes internal packages used as references`, () => {
        ...
        const tsconfigJson = resolvePackageJSONFile(packageName, 'tsconfig.json')
        const internalReferences = tsconfigJson.references || []

        const internalPackages = internalReferences
          .map(internalReference => extractPackagesFromInternalReference(internalReference))
          .sort()
        ...
        const thingsFactoryPackage = dependencies.filter(lib => thingsFactoryReferences.includes(lib)).sort()

        expect(internalPackages).toStrictEqual(thingsFactoryPackage)
      })
    })
  })
})
...
function prefixPackageName(packageName: string) {
  return `@things-factory-sdk/${packageName}`
}
...
function extractPackagesFromInternalReference(internalReference: { path: string }) {
  return prefixPackageName(internalReferenceRegex.exec(internalReference.path).groups.packageName)
}
