const storageAccessHelper = `(function() {
      ...
      StorageAccessHelper.prototype.redirectToAppTLD = function(storageAccessStatus) {
        ...
        data = JSON.stringify({
          message: 'ThingsFactory.API.remoteRedirect',
          data: {
            location: normalizedLink.href,
          }
        });
        window.parent.postMessage(data, this.redirectData.mythingsfactoryUrl);
      }

      StorageAccessHelper.prototype.redirectToAppsIndex = function() {
        window.parent.location.href = this.redirectData.mythingsfactoryUrl + '/admin/apps';
      }
      ...
      StorageAccessHelper.prototype.grantedStorageAccess = function() {
        try {
          sessionStorage.setItem('things_factory.granted_storage_access', true);
          this.setCookie('things_factory.granted_storage_access=true');
          if (!document.cookie) {
      ...
      StorageAccessHelper.prototype.handleHasStorageAccess = function() {
        if (sessionStorage.getItem('things_factory.granted_storage_access')) {
      ...
      StorageAccessHelper.prototype.setUpHelper = function() {
        return new ITPHelper({redirectUrl: window.warehouseOrigin + "/admin/apps/" + window.apiKey + window.returnTo});
      }

      StorageAccessHelper.prototype.setCookieAndRedirect = function() {
        this.setCookie('things_factory.cookies_persist=true');
